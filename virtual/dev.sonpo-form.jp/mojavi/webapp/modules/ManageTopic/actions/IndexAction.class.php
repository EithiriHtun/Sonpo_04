...
require_once(LIB_DIR.'ArticleManager.php');
  ...
  class IndexAction extends DBAction{
    function execute (&$controller, &$request, &$user){
      ...
      $tmp=$article_manager->get_admin_topic_list($year);
      $this_year=(date("Y")+1)*1;
      while(!$tmp && date("Y")*1-$this_year<20){
        $tmp=$article_manager->get_admin_topic_list($this_year--);
      }
      $years=$article_manager->get_topic_list_years();
      
      $data=array();
      if($tmp){
        foreach($tmp as $val){
          if($val["upload_filename"] && file_exists(SONPO_TOPIC_FILE_DIR.$val["upload_filename"].'.html')){
            $val["is_file"]=1;
          }elseif(file_exists(SONPO_TOPIC_FILE_DIR.$val["open_year"]."/".$val["filename"].'.html')){
            $val["is_file"]=1;
          }
