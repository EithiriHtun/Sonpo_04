...
require_once(LIB_DIR.'ArticleManager.php');
  ...
  class IndexAction extends DBAction{
    ...
    function execute (&$controller, &$request, &$user){
      ...
      $tmp=$article_manager->get_admin_en_state_list($year);
      $this_year=(date("Y")+1)*1;
      while(!$tmp && date("Y")*1-$this_year<20){
        $tmp=$article_manager->get_admin_en_state_list($this_year--);
      }

      $data=array();
      if($tmp){
        foreach($tmp as $val){
          if($val["upload_filename"] && file_exists(SONPO_EN_STATE_FILE_DIR.$val["upload_filename"].'.html')){
            $val["is_file"]=1;
          }elseif(file_exists(SONPO_EN_STATE_FILE_DIR.$val["open_year"]."/".$val["filename"].'.html')){
            $val["is_file"]=1;
          }
      ...
      $years=$article_manager->get_en_state_list_years();
