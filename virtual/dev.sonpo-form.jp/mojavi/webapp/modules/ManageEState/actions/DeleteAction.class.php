...
class DeleteAction extends DBAction {
  ...
  function execute(&$controller, &$request, &$user){
    ...
    if($user->getAttribute("is_master")){
      ...
      unlink(SONPO_EN_STATE_FILE_DIR.$data["open_year"]."/".$article_manager->get_filename($id).'.html');
    ...
    $controller->redirect("/manage/index.php/module/ManageEState?y=".$data["open_year"]);
    return VIEW_NONE;
  }
  ...
  function _make_lists(&$controller, &$request, &$user){
    ...
    if($tmp){
      ...
      $renderer->setTemplate('state_list.html');
