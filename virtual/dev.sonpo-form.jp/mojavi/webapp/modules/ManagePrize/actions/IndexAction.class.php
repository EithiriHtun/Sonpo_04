...
require_once(LIB_DIR.'ArticleManager.php');
  ...
  class IndexAction extends DBAction{
    function execute (&$controller, &$request, &$user){
      ...
      $tmp=$article_manager->get_admin_prize_list($year);
      $this_year=(date("Y")+1)*1;
      while(!$tmp && date("Y")*1-$this_year<20){
        $tmp=$article_manager->get_admin_prize_list($this_year--);
      }
      $years=$article_manager->get_prize_list_years();
      
      $data=array();
      if($tmp){
        foreach($tmp as $val){
          if(file_exists(SONPO_PRIZE_FILE_DIR.$val["open_year"]."/".$val["filename"].'.html')){
            $val["is_file"]=1;
          }
