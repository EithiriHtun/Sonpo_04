...
class ImageRegistAction extends DBAction {
  ...
  function getDefaultView(&$controller, &$request, &$user){
    ...
    if(!$request->getErrors()){
      ...
      $user->setAttribute("imagedata",$imagedata);
...
//    require($controller->getModuleDir()."config/config.php");
  ...
  //* for GD
            ...
            $validatorManager->setRequired(
              "invalid_file_type", 
              true, 
            "画像が選択されていません。または、画像ファイルがjpeg形式もしくはgif形式ではありません。".'(image name:'.htmlspecialchars($img_name).',image type:'.htmlspecialchars($img_type).',temp:'.htmlspecialchars($img_tmp_name).',error:'.htmlspecialchars($_FILES['image_'.$i]["error"]).')'
            );
          }
        }
