...
require_once(LIB_DIR.'ArticleManager.php');
  ...
  class ListNewsAction extends DBAction{
    function execute (&$controller, &$request, &$user){
      ...
      while(!file_exists(SONPO_TOPIC_FILE_DIR."list/".sprintf("%04d",$year).'.html')){
        $year--;
      }
      $body = file_get_contents(SONPO_TOPIC_FILE_DIR."list/".sprintf("%04d",$year).'.html');
...
/*
      ...
      $tmp=$article_manager->get_topic_list($year);
