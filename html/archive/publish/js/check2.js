function pre_check(){
	...
	var us=0;
	for(i=0;i<TF.address_type.length;i++){
	  if(TF.address_type[i].selected){
	    us=TF.address_type[i].value;
	  }
	}
...
//---------------------------------------------------------------------------------*/
	...
	var st=0;
	for(i=0;i<TF.send_type.length;i++){
	  if(TF.send_type[i].checked){
	    st=TF.send_type[i].value;
	  }
	}
	if(st==1){
	...
	var sus=0;
	for(i=0;i<TF.saddress_type.length;i++){
	  if(TF.saddress_type[i].selected){
	    sus=TF.saddress_type[i].value;
	  }
	}
	
    if(sus==1){
		//あて先
		if(TF.sname1.value.length == 0 && TF.sname2.value.length == 0){
			alert("お名前が未入力です。\n必須項目となりますのでご入力下さい。");
			TF.sname1.focus();
			return false;
		}
		if(!half_kana_chk(TF.sname1.value) || !half_kana_chk(TF.sname2.value)){
			alert("お名前に半角カタカナが使用されています。\n半角カタカナは使用できません。");
			TF.sname1.focus();
			return false;
		}
		// フリガナ
		if(!full_kana_chk(TF.skana1.value) || !full_kana_chk(TF.skana2.value)){
			alert("フリガナに全角カタカナ以外が使用されています。\n全角カタカナで入力してください。");
			TF.skana1.focus();
			return false;
		}
	...
	if(TF.semail.value.length != 0){
		...
		if(TF.semail.value != TF.email2.value){
			alert("入力されたメールアドレスが一致していません。\n正しく入力してください。");
			TF.semail.focus();
			return false;
		}
