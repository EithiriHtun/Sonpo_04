  ...
  function execute(&$controller, &$request, &$user){
    ...
    switch ($request->getParameter("mode")){
      ...
      case "submit":
        ...
        $controller->redirect("http://www.fits-japan.com/company/recruit/career/entry_thanks.html");
        return VIEW_NONE;
//        return VIEW_SUCCESS;
    }
    return VIEW_NONE;
  ...
  function registerValidators(&$validatorManager, &$controller, &$request, &$user) {
    ...
    if($data["mode"]=="preview"){
      ...
      if(!$data["school_id"] || ($data["school_id"]<1 || $data["school_id"]>8)){
        $validatorManager->setRequired(
          "invalid_school_id", 
          true, 
          "最終学歴の学校種別を選択してください。"
      ...
      $validatorManager->setRequired(
        "grad_id",
        true, 
        "最終学歴の卒業/中退の別を選択してください。"
      );

      if(mb_strlen($data["school_other"])>1000){
        $validatorManager->setRequired(
          "school_other_length",
          true, 
          "その他の学歴が長すぎます。"
      ...
      if(!checkdate($data["c1_smonth"],1,$data["c1_syear"])){
        $validatorManager->setRequired(
          "c1_sdate",
          true, 
      ...
      if(!checkdate($data["c1_emonth"],1,$data["c1_eyear"])){
        $validatorManager->setRequired(
          "c1_edate",
          true, 
      ...
      if($data["wearn_id"] && ($data["wearn_id"]<1 || $data["wearn_id"]>39)){
        $validatorManager->setRequired(
          "invalid_wearn_id", 
          true, 
          "希望年収を正しく選択してください。"
      ...
      if($data["nearn_id"] && ($data["nearn_id"]<1 || $data["nearn_id"]>39)){
        $validatorManager->setRequired(
          "invalid_nearn_id", 
          true, 
          "現年収を正しく選択してください。"
...
/*
      ...
      $validator = new StringValidator($controller);
      $criteria = array(
        'max_error' => "アイテム名：フリガナが長すぎます。",
        'max'       => 64
      );
      $validator->initialize($criteria);
      $validatorManager->register('kana', $validator);
*/
